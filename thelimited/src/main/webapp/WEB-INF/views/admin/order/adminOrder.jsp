...
<div class="container">
	<div class="">
		<div class="mt-5 p-0 ps-5 align-items-center">
			<div class="ps-4">
				<p class="fs-5 fw-bold mb-3 subpage_title">주문관리</p>
...
<!-- 주문정보 표시 영역 -->
				...
				<div class="table-responsive mt-4">
					<table
						class="table border-top table-striped mb-0  fw-light border-bottom">
						<tbody>
											...
											<c:when test="${item.order_id != pre_order_id }">
												<form name="frm_delivery_list">
													...
													<!-- 주문 id -->
																		...
																		<c:choose>
																			<c:when
																				...
																				<select name="s_delivery_state${i.index }"
																					...
																					class="text-warning-emphasis bg-warning-subtle form-select rounded-0 flex-inherit text-start  border border-end text-center">
																					<!-- 배송준비중 option 선택 -->
																			...
																			<c:when
																				...
																				<select name="s_delivery_state${i.index }"
																					id="s_delivery_state${i.index }"
																					onchange="selectValue(this, this.value)"
																					class="text-success bg-success-subtle form-select rounded-0 flex-inherit text-start  border border-end text-center">
																				<!-- 배송중 option 선택 -->
																			...
																			<c:otherwise>
																				...
																				<select name="s_delivery_state${i.index }"
																					id="s_delivery_state${i.index }"
																					onchange="selectValue(this, this.value)"
																					class="text-danger bg-danger-subtle form-select rounded-0 flex-inherit text-start  border border-end text-center">
														...
														<!-- 수정하기 -->
														<input
															class="w-100 btn border-main  rounded-0 samll mb-2"
	...
	$.ajax({
		...
		success : function(data, textStatus) {
			if(data.trim()=='mod_success'){
				//수정완료
				alert("주문 정보를 수정했습니다.");
				location.href="${contextPath}/admin/order/adminOrder.do";
...
//버튼식 상품조회
//onclick할때 같이 가져오는 fixedSearchPeriod값과함께 adminOrder 재요청
...
function search_order_history(fixedSearchPeriod) {
	...
	formObj.method = "get";
	formObj.action = "${contextPath}/admin/order/adminOrder.do";
	formObj.submit();
}
